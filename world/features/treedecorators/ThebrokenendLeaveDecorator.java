...
public class ThebrokenendLeaveDecorator extends LeaveVineDecorator {
	public static Codec<LeaveVineDecorator> CODEC = Codec.unit(ThebrokenendLeaveDecorator::new);
	public static TreeDecoratorType<?> DECORATOR_TYPE = new TreeDecoratorType<>(CODEC);

	@SubscribeEvent
	public static void registerTreeDecorator(RegisterEvent event) {
		event.register(ForgeRegistries.Keys.TREE_DECORATOR_TYPES, registerHelper -> registerHelper.register("thebrokenend_tree_leave_decorator", DECORATOR_TYPE));
	}

	public ThebrokenendLeaveDecorator() {
		super(0.25f);
	}
	...
	private static void addVine(BlockPos pos, Direction direction, TreeDecorator.Context context) {
		context.setBlock(pos, TheBrokenModBlocks.BROKENBLOCK.get().defaultBlockState());
		int i = 4;
		for (BlockPos blockpos = pos.below(); context.isAir(blockpos) && i > 0; --i) {
			context.setBlock(blockpos, oriented(TheBrokenModBlocks.BROKENBLOCK.get().defaultBlockState(), direction));
